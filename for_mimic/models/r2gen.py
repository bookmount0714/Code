...
class R2GenModel(nn.Module):
    ...
    def forward_mimic_cxr(self, images,study_ids=None, targets=None,batch_report_labels=None,images_id=(), mode='train'):
        att_feats, fc_feats = self.visual_extractor(images[:,0])
        att_feats_sam, fc_feats_sam = self.visual_extractor(images[:,1])

        # 将图片数据转化为384维(16,384)
        att_feats_384 = self.imagetransform(att_feats)
        path = globals_ele.path
        ids = globals_ele.study_ids_ann
        reports_tensor = globals_ele.reports_tensor_ann.to(device)
        ...
        titles = [[path[idx] for idx in image_result] for image_result in result.cpu().numpy()]
        features_list = []
        original_lengths_list = []  # 用来保存每个样本的原始长度
        ...
        for title_group in titles:
            ...
            original_lengths_group = []  # 用来保存当前样本的3个特征原始长度
            for image_id in title_group:
                data_entity = globals_ele.radgraph_mimic_data[image_id]['entities']
                g = self.graph_create(image_id, data_entity)
                g = self.graph_node_feature(g, data_entity)
                g = g.to(device)
                node_features = {ntype: g.nodes[ntype].data['feat'] for ntype in
                                 g.ntypes}  # node_features中保存的是原始节点token信息加上种类信息
                ...
                original_length = graph_features.size(0)
                original_lengths_group.append(original_length)
                if graph_features.size(0) < 215:
                    # 计算需要填充的数量
                    padding_needed = 215 - graph_features.size(0)
                    graph_features = F.pad(graph_features, (0, 0, 0, padding_needed), 'constant', value=0)
                group_features_list.append(graph_features)
        ...
        if mode == 'train':
            ...
            features_list = []
            original_lengths_ground = []  # 用于存储每个样本的原始长度
            for image_id in study_ids:
                index = globals_ele.study_ids_ann.index(image_id)
                data_index = path[index]
                data_entity = globals_ele.radgraph_mimic_data[data_index]['entities']
                g = self.graph_create(data_index, data_entity)
                g = self.graph_node_feature(g, data_entity)
                g = g.to(device)
                node_features = {ntype: g.nodes[ntype].data['feat'] for ntype in
            ...
            output = self.encoder_decoder(fc_feats,fc_feats_sam,att_feats,att_feats_sam, targets,graph_feature,graph_feature_ground,study_ids,result,top_3_values,self.tokenizer,att_feats_384,att_feats_41,batch_report_labels,original_lengths_tensor,original_lengths_tensor_ground, mode='forward')
        elif mode == 'sample':
            output, _ = self.encoder_decoder(fc_feats,fc_feats_sam, att_feats, att_feats_sam,graph_feature,result,top_3_values,self.tokenizer,original_lengths_tensor, mode='sample')
        else:
            raise ValueError
