...
class MimiccxrSingleImageDataset(BaseDataset):
    def __getitem__(self, idx):
        ...
        image_id = example['id']
        image_path = example['image_path']
        image_path_sam = image_path[0].rstrip('.jpg') + '_sam.jpg'
        image = Image.open(os.path.join(self.image_dir, image_path[0])).convert('RGB')
        image_sam = Image.open(os.path.join(self.image_dir, image_path_sam)).convert('RGB')
        if self.transform is not None:
            image = self.transform(image)
            image_sam = self.transform(image_sam)
        images = torch.stack((image, image_sam), 0)
        study_ids = example['study_id']
        report_ids = example['ids']
        report_masks = example['mask']
        seq_length = len(report_ids)
        sample = (image_id,study_ids , images, report_ids, report_masks, seq_length)
        return sample
