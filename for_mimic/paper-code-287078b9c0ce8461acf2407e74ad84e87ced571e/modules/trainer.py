...
#mimic
    def _train_epoch(self, epoch):
        ...
        globals_ele.reports_tensor_ann = torch.from_numpy(self.model.sentence_bert.encode(globals_ele.reports_ann))
        #一个迭代开始前对报告进行分类
        embedding = self.model.reducer.fit_transform(globals_ele.reports_tensor_ann)
        ...
        train_loss = 0
        self.model.train()
        for batch_idx, (images_id,study_ids, images, reports_ids, reports_masks) in enumerate(self.train_dataloader):
            ...
            for id in study_ids:
                try:
                    index = globals_ele.study_ids_ann.index(id)
            ...
            output = self.model(images,study_ids, reports_ids,batch_report_labels,images_id ,mode='train')
        ...
        with torch.no_grad():
            val_gts, val_res = [], []
            for batch_idx, (images_id, study_ids,images, reports_ids, reports_masks) in enumerate(self.val_dataloader):
        ...
        with torch.no_grad():
            test_gts, test_res = [], []
            for batch_idx, (images_id,study_ids, images, reports_ids, reports_masks) in enumerate(self.test_dataloader):
                images, reports_ids, reports_masks = images.to(self.device), reports_ids.to(
                    self.device), reports_masks.to(self.device)
                output = self.model(images,images_id = images_id,mode='sample')
                reports = self.model.tokenizer.decode_batch(output.cpu().numpy())
